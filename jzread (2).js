...
// [task_local]
// * */1 * * * http://nm66.top/jzread.js, tag=柠檬九章头条, img-url=circles.hexagongrid.fill.system, enabled=true
...
let jzreadurls = ""
let jzurl='https://api.st615.com/v1/user/info?token='
        ...
        if (jzreadurls[item]) {
          ...
          console.log(`共${jzreadurlArr.length}个cookie`)
	        for (let k = 0; k < jzreadurlArr.length; k++) {
                $.message = ""
                jzreadurl = jzurl+jzreadurlArr[k]
                $.index = k + 1;
          console.log(`\n开始【九章头条${$.index}】`)
